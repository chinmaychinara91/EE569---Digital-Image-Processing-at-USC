...
int main(int argc, char* argv[])
{
	...
	if (argc != 8)	{		cout << "-----------------------------------------------------------------------------------------------" << endl;		cout << "Syntax Error - Incorrect parameter usage" << endl;		cout << "-----------------------------------------------------------------------------------------------" << endl;		cout << "Make sure the parameters passed are in the following format:" << endl;
		...
		cout << "gauss_image: the name of the .raw file along with path for storing gaussian image" << endl;
		cout << "gauss_hist: the name of the .txt file along with path to which the gaussian image histogram data is saved" << endl;
	...
	if (file != NULL)
	{
		...
		fclose(file);
		cout << "File " << "Hist.txt" << " written successfully !!!" << endl;
	}
	...
	/* Equalize the histogram matched data for better visibility*/
	double Prob[256][3];
	...
	for (int i = 0;i < 256;i++)
	{
		for (int k = 0;k < 3;k++)
		{
			HistMatch[i][k]; // number of pixels
			Prob[i][k] = HistMatch[i][k] / (1.0 * ImgHeight * ImgWidth); // Probability
			if (i == 0)
				CumProb[i][k] = Prob[i][k];
			else
				CumProb[i][k] = CumProb[i - 1][k] + Prob[i][k];
			NewPixelVal[i][k] = CumProb[i][k] * 255.0;
			NewPixelVal[i][k] = floor(NewPixelVal[i][k]);
		}
	}
	...
	for (int r = 0;r < 256;r++)
	{
		for (int i = 0;i < ImgHeight; i++)
		{
			for (int j = 0;j < ImgWidth; j++)
			{
				for (int k = 0;k < BytesPerPixel;k++)
				{
					HistEqlImage3d[i][j][k] = NewPixelVal[HistMatchImage3d[i][j][k]][k];
				}
			}
		}
	}
	...
	/* store histogram equalised data */
	unsigned long HistEql[256][3];
	Histogram3d(HistEql, HistEqlImage3d, ImgHeight, ImgWidth, BytesPerPixel);
	FileWriteHist3d("histmatch_equal.txt", HistEql);
